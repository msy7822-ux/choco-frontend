  ...
  return (
    <>
      <Layout>
        ...
        <Box sx={{ mt: '1.2rem', px: '1.1rem' }}>
          {/* 撮影＆画像アップロード */}
          <Box sx={{ display: 'flex', justifyContent: 'center' }}>
            {imageUrl.length !== 0 &&
              // 画像がアップされたら、その画像を載せた枠を用意する
            ...
            {/* 空いた部分にImageDisplayerを配置する   */}
            {generateImageDisplayer()}
            {imageUrl.length < 3 &&
              // 画像が3枚掲載されたら、アップロードできないようにアイコンを表示しない
              <ImageUploader
                imageUrl={imageUrl}
                setImageUrl={setImageUrl}
                setIsMaxImage={setIsMaxImage}
              />
            }
          </Box>
